...
public class LeapManager : MonoBehaviour 
{
	...
	public Pointable GetLeftFinger ()
	{
		if ((leapPointable != null) && leapPointable.IsValid)
		{
			Pointable leftfinger = leapFrame.Pointables[0];
			foreach (Pointable finger in leapFrame.Pointables)
			{
				if (finger.TipPosition.x < leftfinger.TipPosition.x)
					leftfinger = finger;
			}
			return leftfinger;
		}
	...
	public Pointable GetRightFinger ()
	{
		if ((leapPointable != null) && leapPointable.IsValid)
		{
			Pointable rightFinger = leapFrame.Pointables[0];
			foreach (Pointable finger in leapFrame.Pointables)
			{
				if (finger.TipPosition.x > rightFinger.TipPosition.x)
					rightFinger = finger;
			}
			return rightFinger;
		}
